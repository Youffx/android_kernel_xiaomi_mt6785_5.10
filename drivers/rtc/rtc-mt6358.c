...
/* boot mode values (from mtk_boot_common.h) */
#define BOOT_MODE_NORMAL			0
#define BOOT_MODE_KERNEL_POWER_OFF_CHARGING	8
#define BOOT_MODE_LOW_POWER_OFF_CHARGING	9
...
static void rtc_mark_kpoc(void)
{
	unsigned long flags;

	spin_lock_irqsave(&mt_rtc->lock, flags);
	regmap_update_bits(mt_rtc->regmap, mt_rtc->addr_base + RTC_PDN1,
			   RTC_PDN1_KPOC, RTC_PDN1_KPOC);
...
static irqreturn_t mtk_rtc_irq_handler(int irq, void *data)
{
	...
	if (pwron_alarm) {
		...
		if (now_time >= time - 1 && now_time <= time + 4) {
			if (boot_mode == BOOT_MODE_KERNEL_POWER_OFF_CHARGING
			    || boot_mode == BOOT_MODE_LOW_POWER_OFF_CHARGING) {
				mtk_rtc_reboot();
				spin_unlock_irqrestore(&mt_rtc->lock, flags);
...
static int mtk_rtc_pdrv_probe(struct platform_device *pdev)
{
	...
	if (rtc->irq > 0)
		...
		pr_notice("%s: no IRQ, alarms disabled\n", __func__);
	...
	device_init_wakeup(&pdev->dev, rtc->irq_valid);
