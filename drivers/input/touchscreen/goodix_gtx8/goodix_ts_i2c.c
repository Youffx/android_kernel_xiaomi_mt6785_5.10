				...
				u8 *buf, int size)
{
	...
	for (i = 0; i < sub_bags; i++) {
		...
		if (offset + 2 >= size) {
			ts_err("config size overflow, %d", offset + 2);
			ret = -EINVAL;
			goto err_out;
		}
				...
				u8 *buf, int size)
{
	...
	for (i = 0; i < sub_bags; i++) {
		...
		ts_debug("sub bag num:%u,sub bag length:%u",
			 buf[offset], subbag_len);
		if (offset + 2 + subbag_len + 1 >= size) {
			ts_err("config size overflow, %d", offset + 2 + subbag_len + 1);
			ret = -EINVAL;
			goto err_out;
		}
		ret = goodix_i2c_read(dev, cfg_addr + offset + 2,
			      ...
			      u8 *config_data, int size)
{
	...
	if (dev->ic_type == IC_TYPE_YELLOWSTONE)
		r = goodix_read_config_ys(dev, config_data, size);
	else
		r = goodix_read_config_nor(dev, config_data, size);
	if (r <= 0)
...
static int goodix_capacitance_data(struct goodix_ts_device *dev,
		struct ts_rawdata_info *info)
{
	struct goodix_ts_core *cd = goodix_modules.core_data;
	int retry = 20;
	u16 rawdata_addr = 0x8FA0;
	u16 diffdata_addr = 0x9D20;
	u32 tx = cd->drv_num;
	u32 rx = cd->sen_num;
	int size = tx * rx;
	u8 irq_flag;
	...
	if (!info) {
		ts_err("goodix_capacitance_data, input null ptr");
		return -EIO;
	}

	/* disable irq & close esd */
	goodix_ts_blocking_notify(NOTIFY_ESD_OFF, NULL);
	goodix_ts_irq_enable(cd, false);
	usleep_range(10000, 11000);
	...
	cmds[1] = 0x00;
	cmds[2] = (u8)(0x00 - 0x01);
	ret = goodix_i2c_write(dev, dev->reg.command, cmds, sizeof(cmds));
	if (ret < 0) {
		ts_err("switch rawdata mode failed");
		goto exit;
	}
	msleep(30); //wait command finish

	/* clean touch event flag */
	irq_flag = 0;
	ret = goodix_i2c_write_trans(dev, dev->reg.coor, &irq_flag, 1);
	if (ret < 0) {
	...
	while (retry--) {
		...
		ret = goodix_i2c_read_trans(dev, dev->reg.coor, &irq_flag, 1);
		if (!ret && (irq_flag & 0x80))
			break;
	}
	if (retry < 0) {
		ts_err("rawdata is not ready irq_flag:0x%02x, exit!", irq_flag);
		goto exit;
	}
	...
	info->buff[1] = tx;
	info->used_size = 2;
	ret = goodix_i2c_read_trans(dev, rawdata_addr,
			(u8 *)&info->buff[info->used_size], size * sizeof(u16));
	if (ret < 0) {
	...
	for (i = 0; i < size; i++)
		info->buff[info->used_size + i] =
			be16_to_cpu(info->buff[info->used_size + i]);
	goodix_rotate_abcd2cbad(tx, rx, &info->buff[info->used_size]);
	info->used_size += size;
	
	ret = goodix_i2c_read_trans(dev, diffdata_addr,
			(u8 *)&info->buff[info->used_size], size * sizeof(u16));
	if (ret < 0) {
...
exit:
	...
	cmds[1] = 0;
	cmds[2] = 0;
	goodix_i2c_write_trans(dev, dev->reg.command, cmds, sizeof(cmds));
	/* clean touch event flag */
	irq_flag = 0;
	goodix_i2c_write_trans(dev, dev->reg.coor, &irq_flag, 1);
	msleep(30); //wait command finish
	/* enable irq * esd */
	goodix_ts_irq_enable(cd, true);
...
static const struct goodix_ts_hw_ops hw_i2c_ops = {
	...
	.get_capacitance_data = goodix_capacitance_data,
};
...
static const struct of_device_id i2c_matchs[] = {
	...
	{.compatible = "goodix,gt6861",},
	{.compatible = "goodix,gt6862",},
	{},
};
