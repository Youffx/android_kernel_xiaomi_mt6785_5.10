...
struct bq2597x_cfg {
	...
	int sc8551_bypass_in_fcc;
	int sc8551_bypass_out_fcc;
};
...
int sc8551_set_charge_mode(struct bq2597x *bq, int mode)
{
	int ret;
	u8 val;
...
int sc8551_get_charge_mode(struct bq2597x *bq)
{
	int ret;
	u8 val;
...
static int bq2597x_check_vbus_error_status(struct bq2597x *bq)
{
	...
	ret = bq2597x_read_byte(bq, BQ2597X_REG_0A, &stat);
	if (!ret) {
		bq_info("BQ2597X_REG_0A:0x%2x\n", stat);
		if (stat & VBUS_ERROR_LO)
			return VBUS_ERROR_LOW;
		else if (stat & VBUS_ERROR_HI)
