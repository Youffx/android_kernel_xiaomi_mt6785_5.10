 ...
 *=============================================================
 */
static struct wcn_platform_bridge bridge;
...
EXPORT_SYMBOL(wcn_export_platform_bridge_register);
void wcn_export_platform_bridge_unregister(void)
{
	memset(&bridge, 0, sizeof(struct wcn_platform_bridge));
}
EXPORT_SYMBOL(wcn_export_platform_bridge_unregister);
int mtk_wcn_temp_query_ctrl(void)
{
	if (unlikely(!bridge.thermal_query_cb)) {
		pr_debug("E_WF:Thermal query not register\n");
		return -1;
	} else {
...
static int (*ts_get_temp_wrap[4]) (void) = {
	mtk_wcn_temp_query_ctrl /* 0 is for WMT sensor */
...
set_adaptive_wifi_tput_limit(unsigned int cur_tput, unsigned int limit_ratio)
{
	...
	if (limit_ratio == -1) {
		...
		limit_tput =
			(prev_limit_tput * limit_ratio) / init_wifi_tput_ratio;
...
static int adaptive_tput_ratio(long prev_temp, long curr_temp)
{
	...
	if (cl_dev_adp_cpu_state_active == 1) {
		...
		if (((curr_temp > WIFI_TARGET_TJ_HIGH)
			...
			wifi_tput_ratio +=
			      (tt_wifi / tt_wifi_high + tp_wifi / tp_wifi_rise);
		} else if (
			((curr_temp > WIFI_TARGET_TJ_HIGH)
				&& (curr_temp <= prev_temp)) ||
			((curr_temp <= WIFI_TARGET_TJ_HIGH)
			...
			wifi_tput_ratio +=
			      (tt_wifi / tt_wifi_high + tp_wifi / tp_wifi_fall);
		} else if ((curr_temp < WIFI_TARGET_TJ_LOW)
				   && (curr_temp > prev_temp)) {
			wifi_tput_ratio +=
			       (tt_wifi / tt_wifi_low + tp_wifi / tp_wifi_rise);
		} else if ((curr_temp < WIFI_TARGET_TJ_LOW)
				   && (curr_temp <= prev_temp)) {
			wifi_tput_ratio +=
			       (tt_wifi / tt_wifi_low + tp_wifi / tp_wifi_fall);
		}
	...
	if (wifi_throttle_version == 0) {
		...
		case 0:	/* wifi last */
			if (is_cpu_power_unlimit()) {
				...
				cl_dev_adp_cpu_state_active = (temp >=
					(wifi_target_tj - wifi_target_offset)) ?
					1 : 0;
				...
				if (!is_wifi_tput_min()) {
					set_adaptive_wifi_tput_limit(
					    tx_throughput, min_wifi_tput_ratio);
				}
				triggered = 1;	/* wt2 */
			}
			break;
		...
		case 3:	/* independent */
			 ...
			 */
				...
				wifi_target_tj = min(wmt_wifi_target_tj,
					get_cpu_target_tj()
			...
			adaptive_tput_ratio(g_prev_temp, temp);
			break;
		/* not share target tj (default): for non-integrated WiFi */
		case 4:
		default:
			wifi_target_tj = wmt_wifi_target_tj;
			wifi_target_offset = wmt_wifi_target_offset;
...
static int wmt_thz_get_trip_temp(struct thermal_zone_device *thz_dev,
							int trip, int *pv)
{
...
static inline unsigned long thro(unsigned long a, unsigned int b,
								unsigned int c)
{
	unsigned long tmp;
...
static int wmt_judge_throttling(int index, int is_on, int interval)
{
	...
	case HIGH_STAT:
		if (throttling_pre_stat < HIGH_STAT ||
			...
			/* At this moment, current throughput is none.
			 * Use the previous constraint.
			 */
			else
				thro_constraint =
				    thro(thro_constraint,
					 up_numerator, up_denominator);
				...
				if (cur_thro < thro_constraint)
					thro_constraint =
					    thro(cur_thro, up_numerator,
								up_denominator);
				/* current throughput is large than constraint.
				 * WHAT!!!
				 */
				else
					thro_constraint =
								...
								unsigned long v)
{
	...
	wmt_tm_dprintk("[%s] wifi_throttle_version=%u,cl_pa1_dev_state=%u\n",
			__func__, wifi_throttle_version, cl_pa1_dev_state);
...
/* cl_pa1_dev_state = (unsigned int)v; */
		...
		wmt_tm_dprintk("[%s] cl_pa1_dev_state=%u,wif curr T=%d\n",
				__func__, cl_pa1_dev_state, g_curr_temp);
...
/* New Wifi throttling Algo+ */
ssize_t wmt_wifi_algo_write(struct file *filp, const char __user *buf,
						size_t len, loff_t *data)
{
	...
	if (sscanf
		...
		wmt_tm_printk("[%s] %s [up]%d %d/%d, [low]%d %d/%d, rst=%d\n",
				__func__, desc, up_duration, up_numerator,
				up_denominator, low_duration, low_numerator,
...
int wmt_wifi_algo_read(struct seq_file *m, void *v)
{
	...
	wmt_tm_printk("[%s] [up]%d %d/%d, [low]%d %d/%d, rst=%d\n",
			__func__, up_duration, up_numerator, up_denominator,
			low_duration, low_numerator, low_denominator,
			low_rst_max);
...
ssize_t wmt_tm_wfd_write(struct file *filp, const char __user *buf, size_t len,
								loff_t *data)
{
	int ret = 0;
	char tmp[MAX_LEN] = { 0 };
						...
						size_t len, loff_t *data)
{
	...
	wmt_tm_printk("qqq %s ret :%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d"
		      , __func__, ret,
		      wifi_throttle_version,
		      sensor_select,
		      resource_allocator_policy,
...
static int wmt_tm_thz_cl_register(void)
{
	...
	/* cooling devices */
	p_linux_if->cl_dev = mtk_thermal_cooling_device_register(
			"mtktswmt-sysrst", NULL, &mtktspa_cooling_sysrst_ops);

	p_linux_if->cl_pa1_dev = mtk_thermal_cooling_device_register(
			"mtktswmt-pa1", NULL, &mtktspa_cooling_pa1_ops);

	p_linux_if->cl_pa2_dev = mtk_thermal_cooling_device_register(
			"mtktswmt-pa2", NULL, &mtktspa_cooling_pa2_ops);

	p_linux_if->interval = DEFAULT_POLL_TIME;

	/* trips */
	p_linux_if->thz_dev = mtk_thermal_zone_device_register(
			"mtktswmt", g_num_trip, NULL, &wmt_thz_dev_ops,
			0, 0, 0, p_linux_if->interval);
						...
						size_t count, loff_t *data)
{
	...
	if (sscanf(ptr_tm_data->desc,
		...
		g_bind0[0] = g_bind1[0] = g_bind2[0] = g_bind3[0] = '\0';
		g_bind4[0] = g_bind5[0] = g_bind6[0] = g_bind7[0] = '\0';
		...
		wmt_tm_dprintk
		    ...
		    , __func__,	g_thermal_trip[5], g_thermal_trip[6],
		...
		wmt_tm_dprintk(
			...
			__func__, g_bind5, g_bind6, g_bind7, g_bind8, g_bind9);
...
static int wmt_tm_proc_register(void)
{
	...
	if (!wmt_thro_proc_dir) {
		...
		proc_create("tx_thro", 0644, wmt_thro_proc_dir,
				&_tx_thro_fops);
		proc_create("tx_thro_limit", 0644,
				wmt_thro_proc_dir, &_tx_thro_limit_fops);
	}
	...
	if (!wmt_tm_proc_dir) {
		...
		entry = proc_create("clwmt_pid",
				0660, wmt_tm_proc_dir, &_tm_pid_fops);
		if (entry)
			proc_set_user(entry, uid, gid);

		entry =  proc_create("clwmt_val",
				0660, wmt_tm_proc_dir, &_wmt_val_fops);
		if (entry)
			proc_set_user(entry, uid, gid);

		entry = proc_create("clwmt_wfdstat",
				0660, wmt_tm_proc_dir, &_wfd_stat_fops);
		if (entry)
			proc_set_user(entry, uid, gid);

		entry = proc_create("wifi_in_soc",
				0660, wmt_tm_proc_dir, &_wifi_in_soc_fops);
		if (entry)
			proc_set_user(entry, uid, gid);
	}
	return 0;
