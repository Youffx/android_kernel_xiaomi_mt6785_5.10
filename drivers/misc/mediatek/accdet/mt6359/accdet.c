...
#if !defined CONFIG_MTK_PMIC_NEW_ARCH || !defined AUXADC_LIST_ACCDET
#define AUXADC_LIST_ACCDET 0
static int pmic_get_auxadc_value(int list)
{
	return 0;
}
#endif
...
void __weak pmic_register_interrupt_callback(unsigned int intNo,
	void(*func)(void))
{
}

void __weak pmic_enable_interrupt(enum PMIC_FAKE_IRQ_ENUM intNo,
	unsigned int en, char *str)
{
}

unsigned int __weak pmic_Read_Efuse_HPOffset(int i)
{
	return 0;
}
...
#ifdef CONFIG_MTK_PMIC_WRAP
static struct regmap *accdet_pwrap_node_to_regmap(struct device_node *np)
{
	...
	return dev_get_regmap(&pdev->dev, NULL);
}
#endif
...
static int accdet_get_dts_data(void)
{
	...
	if (accdet_node) {
		accdet_regmap = accdet_pwrap_node_to_regmap(accdet_node);
