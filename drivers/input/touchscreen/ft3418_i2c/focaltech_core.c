...
// Youffx: disabled for 5.10 — mtkfb_lcm_name is not populated by DRM v2
...
static int fts_power_source_init(struct fts_ts_data *ts_data)
{
    ...
    ts_data->vdd = regulator_get(ts_data->dev, "vtouch");
    if (IS_ERR_OR_NULL(ts_data->vdd)) {
                                ...
                                unsigned long event, void *data)
{
    ...
    case FB_DRM_BLANK_UNBLANK:
        if (FB_DRM_EARLY_EVENT_BLANK == event) {
            FTS_INFO("resume: event = %lu, not care\n", event);
        } else if (FB_DRM_EVENT_BLANK == event) {
			FTS_INFO("FB_DRM_EVENT resume");
            queue_work(fts_data->ts_workqueue, &fts_data->resume_work);
        }
        break;
    case FB_DRM_BLANK_POWERDOWN:
        if (FB_DRM_EARLY_EVENT_BLANK == event) {
			FTS_INFO("FB_DRM_EVENT ");
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
...
#if !defined(CONFIG_FB) && defined(CONFIG_DRM) && defined(CONFIG_DRM_PANEL)
    ...
    ret = fb_drm_register_client(&ts_data->fb_notif);
    if (ret) {
...
static int __init fts_ts_init(void)
{
    ...
    if (IS_ERR_OR_NULL(mtkfb_lcm_name)){
        ...
        if (strcmp(mtkfb_lcm_name,"dsi_panel_k7_38_0c_0a_fhdp_video") == 0) {
