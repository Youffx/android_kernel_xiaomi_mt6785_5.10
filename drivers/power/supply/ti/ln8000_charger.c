...
static u8 current_fsw = 9;
/* 2021.03.26 longcheer jiangshitian HMI_M306_A01-626 end */
extern void set_ln8000_load_flag(bool lnflag);
...
static void ln8000_print_regmap(struct ln8000_info *info)
{
	...
	ln8000_read_reg(info, 0x3A, &regs[0]);
	ln_info("0x3A[0x%02X]\n", regs[0]);
}
...
static void ln8000_soft_reset(struct ln8000_info *info)
{
    ...
    /* keep fsw_cfg OTP value for tune switching frequency */
    ln8000_read_reg(info, LN8000_REG_SYS_CTRL, &reg);
    info->fsw_cfg_otp = (reg >> 4) & 0xF;

    ln_info("fsw_cfg_otp = 0x%x\n", info->fsw_cfg_otp);
}
/* 2021.03.26 longcheer jiangshitian HMI_M306_A01-626 end */
...
static int ln8000_psy_register(struct ln8000_info *info)
{
    ...
    info->psy_desc.name 		= (LN8000_IS_PRIMARY(info) ? "ln8000" : "ln8000-secondary");
...
/* 2021.03.26 longcheer jiangshitian HMI_M306_A01-626 start */
static ssize_t ln8000_show_fsw(struct device *dev,
...
static ssize_t ln8000_store_fsw(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct ln8000_info *ln8000 = dev_get_drvdata(dev);
	int ret;
	unsigned int val;

	ret = sscanf(buf, "%d", &val);

	ln8000_set_sw_freq(ln8000, val);//0---7[>800khz step:20] 8---15][>400khz step:40]

	return count;
}

static DEVICE_ATTR(ln8000_fsw, 0660, ln8000_show_fsw, ln8000_store_fsw);

static struct attribute *ln8000_attributes[] = {
	&dev_attr_ln8000_fsw.attr,
	NULL,
};

static const struct attribute_group ln8000_attr_group = {
	.attrs = ln8000_attributes,
};
/* 2021.03.26 longcheer jiangshitian HMI_M306_A01-626 end */
...
static int ln8000_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* 2021.03.26 longcheer jiangshitian HMI_M306_A01-626 start */
	ret = sysfs_create_group(&info->dev->kobj, &ln8000_attr_group);
	if (ret) {
		pr_err("ln8000 failed to register sysfs. err: %d\n", ret);
		goto err_wakeup;
	}
	/* 2021.03.26 longcheer jiangshitian HMI_M306_A01-626 end */
...
static int ln8000_remove(struct i2c_client *client)
{
	...
	/* 2021.03.26 longcheer jiangshitian HMI_M306_A01-626 start */
	sysfs_remove_group(&info->dev->kobj, &ln8000_attr_group);
	/* 2021.03.26 longcheer jiangshitian HMI_M306_A01-626 end */
